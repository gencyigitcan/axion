-- FINAL FIX: Schema & Massive Seed (Dynamic SQL to avoid Parse Errors)
-- This script ensures columns exist AND inserts data safely.

-- 1. Ensure Columns Exist (DDL)
alter table user_profiles add column if not exists phone text;
alter table user_profiles add column if not exists birth_date date;

-- 2. Run Massive Seed using Dynamic SQL
DO $$
DECLARE
    v_tenant_id uuid;
...
BEGIN
    ...
    FOR v_i IN 1..100 LOOP
        ...
        -- Create Profile using DYNAMIC SQL to bypass parser checks if columns were just added

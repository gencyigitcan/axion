-- FINAL ATTEMPT v6: Extreme Resilience
-- Error 42703: duration_minutes in class_types missing.
-- We will use DYNAMIC SQL for creating metadata too, so it adapts to whatever schema exists.
-- Also we will attempt to add missing columns before running.
...
alter table class_types add column if not exists duration_minutes integer default 60; -- ADDED THIS
...
BEGIN
    ...
    -- 2. SELF-HEALING: Create Class Types if missing (Dynamic)

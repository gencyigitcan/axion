-- v8: The Immortal Script
-- Error 42703: credit_amount in packages missing (Likely named 'credits' or similar, let's standardize on credit_amount)
...
alter table packages add column if not exists credit_amount integer default 10; -- ADDED
alter table packages add column if not exists price numeric(10,2); -- Ensure price exists too
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM packages WHERE tenant_id = v_tenant_id) THEN
        v_pkg_query := format('INSERT INTO packages (tenant_id, name, credit_amount, price, validity_days) VALUES (%L, %L, 8, 2000, 30)', v_tenant_id, '8 Derslik Paket');
        EXECUTE v_pkg_query;
        v_pkg_query := format('INSERT INTO packages (tenant_id, name, credit_amount, price, validity_days) VALUES (%L, %L, 12, 2800, 45)', v_tenant_id, '12 Derslik Paket');
        EXECUTE v_pkg_query;
    END IF;

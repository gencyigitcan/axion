-- FINAL ROBUST FIX: Schema Corrections & Dynamic Partial Seeding
-- 1. Fix Missing Columns
alter table user_profiles add column if not exists phone text;
alter table user_profiles add column if not exists birth_date date;
alter table user_credits add column if not exists expire_date timestamptz;

-- 2. Make package_id nullable if it's strictly required, to allow manual credit adjustments without package
-- (Or we ensure we provide one. For now, let's just ensure we can insert.)
alter table user_credits alter column package_id drop not null;

-- 3. Run Seed with Dynamic SQL
DO $$
DECLARE
    v_tenant_id uuid;
...
BEGIN
    ...
    FOR v_i IN 1..100 LOOP
        ...
        v_credit_query := format(
            'INSERT INTO user_credits (tenant_id, user_id, remaining_credits, status, expire_date, package_id) VALUES (%L, %L, %s, %L, %L, %L)',
            v_tenant_id,
            v_student_id,
        ...
        -- MEASUREMENTS (Standard Insert, table created recently so likely fine)

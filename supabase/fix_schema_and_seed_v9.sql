-- v9: The Corrected Script (Targeting 'credit_count')
-- Error 23502 revealed the real column name is 'credit_count' in packages table.
...
-- Note: 'packages' has 'credit_count', not 'credit_amount'. We won't add 'credit_amount'.
...
BEGIN
    ...
    -- 3. SELF-HEALING: Create Packages (USING credit_count)
    IF NOT EXISTS (SELECT 1 FROM packages WHERE tenant_id = v_tenant_id) THEN
        -- Using 'credit_count' as discovered from error log
        v_pkg_query := format('INSERT INTO packages (tenant_id, name, credit_count, price, validity_days) VALUES (%L, %L, 8, 2000, 30)', v_tenant_id, '8 Derslik Paket');
        EXECUTE v_pkg_query;
        v_pkg_query := format('INSERT INTO packages (tenant_id, name, credit_count, price, validity_days) VALUES (%L, %L, 12, 2800, 45)', v_tenant_id, '12 Derslik Paket');
        EXECUTE v_pkg_query;
    END IF;

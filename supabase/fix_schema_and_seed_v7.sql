-- v7: The Undying Script
-- Error 42703: credits_required missing in class_types.
...
alter table class_types add column if not exists credits_required integer default 1; -- ADDED
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM class_types WHERE tenant_id = v_tenant_id) THEN
        -- Use dynamic SQL to insert even if columns were just added
